...
export default function EditArticlePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!article) return <div className="p-8">加载中...</div>;
...
function ArticleEditor({ initial }: { initial: any }) {
  ...
  async function handleSave(draft: boolean) {
    ...
    await fetch(`/api/articles/${initial.id}`, {
      ...
      body: JSON.stringify({ title, category, date, summary, content, cover, published: !draft }),
    });
    setSaving(false);
    router.push("/admin");
  }
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-brown mb-6">编辑文章</h1>
